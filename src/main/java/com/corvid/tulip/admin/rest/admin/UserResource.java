...
@Path("/users")
@javax.ejb.Stateless
public class UserResource extends TulipBaseEntityResource<User> {
    ...
    public Response createState(@Context HttpHeaders headers, GenericDTO entityDTO) {
        ...
        try {
            ...
            User entity = genericDTOUtil.fromDTO(entityDTO, User.class);
            final String salt = BCrypt.gensalt();
            entity.setPassword(BCrypt.hashpw(entity.getEmail(), salt));
            ...
            builder = Response.created(URI.create(uriInfo.getAbsolutePath() + "/" + createdState.getId().toString()))
                    .entity(genericDTOUtil.toDTO(createdState, User.USER_SESSION_ATTRIBUTES, User.class));
    ...
    public Response changePassword(@Context HttpHeaders headers, @PathParam("id") String id,
        ...
        if(!newPassword.equals(confirmPassword) || !BCrypt.checkpw(currentPassword, u.getPassword())){
            log.info("passwords do not conform or current password not correct...." + !newPassword.equals(confirmPassword) + ", " + !BCrypt.checkpw(currentPassword, u.getPassword()));
