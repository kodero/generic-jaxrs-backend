...
public class UserRepository implements Serializable {
    ...
    */
    ...
    public AuthToken findAuthToken(String token) {
        String query = "select authToken from AuthToken authToken where authToken.accessToken = :token";
        List<AuthToken> authTokens = gService
                .createQuery(query)
                .setParameter("token", token)
                .getResultList();
        if(authTokens.size() == 1) return authTokens.get(0);
        return null;
    }
